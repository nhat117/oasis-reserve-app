...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ─── Send handoff notice to customer via Sinch ──────────────────
    if (conversation_id) {
      try {
        ...
        if (convo?.external_conversation_id) {
          const handoffMessage = "You've been connected with our team. A staff member will respond shortly.";
          await fetch(`${supabaseUrl}/functions/v1/sinch-send-message`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
          ...
          }).catch((e) => console.error("Sinch handoff message failed:", e));

          notifiedVia.push("sinch_message");
        }
      } catch (sinchErr) {
        console.error("Sinch handoff notification failed:", sinchErr);
      }
    }
